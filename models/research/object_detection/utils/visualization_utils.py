...
STANDARD_COLORS = [
    'Aqua', 'LightBlue', 'LightGreen', 'Orange', 'Azure', 'Beige', 'Bisque',
...
def draw_bounding_box_on_image(image,
  ...
  try:
    ...
    #font = ImageFont.truetype('fonts/FreeMono.ttf', 24, encoding="unic")
...
def visualize_boxes_and_labels_on_image_array(image,
  ...
  for i in range(min(max_boxes_to_draw, boxes.shape[0])):
    if scores is None or scores[i] > min_score_thresh:
      ...
      if scores is None:
        ...
        list_ymin.append(int(box[0]*im_h))
        list_xmin.append(int(box[1]*im_w))
        list_ymax.append(int(box[2]*im_h))
        list_xmax.append(int(box[3]*im_w))
        list_color.append(STANDARD_COLORS[classes[i] % len(STANDARD_COLORS)])
  ...
  full_dataframe = pd.DataFrame({'Class': list_class, 'Inference': list_score,
                                'Color': list_color, 'Xmin': list_xmin, 'Ymin': list_ymin,
                                'Xmax': list_xmax, 'Ymax': list_ymax})

  return image, full_dataframe
